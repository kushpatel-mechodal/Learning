...
if ($action === "create_school") {
    ...
    $sql = "INSERT INTO schools (district_id,taluka_id,school_name) VALUES(?,?,?)";
    ...
    mysqli_stmt_bind_param($stmt, "iis", $district_id, $taluka_id, $school_name);

    if (mysqli_stmt_execute($stmt)) {

        $school_id = mysqli_insert_id($conn);
        http_response_code(201);
        $reponse = ["message" => "school created Successfully", "status" => true, "district_id" => $school_id];
        exit;
    } else {
        http_response_code(500);
        $reponse = ["message" => "Error for Create school", "status" => false];
        exit;
    }

} else if ($action === "read_school") {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $reponse = ["message" => "School Data Fetch Successfully ", "status" => true, "data" => $output];
            exit;
        } else {
        ...
        http_response_code(500);
        $reponse = ["message" => "Failed to Fetch school data ", "status" => false];
        exit;
    }
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if ($affected_row > 0) {
            http_response_code(200);
            $reponse = ["message" => "School data deleted successfully", "status" => true];
            exit;
        } else {
            http_response_code(404);
            $reponse = ["message" => "Data not found", "status" => false];
    ...
    $sql = "UPDATE schools SET district_id=?,taluka_id=?,school_name=? where id=?";

    $stmt = mysqli_prepare($conn, $sql);

    mysqli_stmt_bind_param($stmt, "iisi", $district_id, $taluka_id, $school_name, $id);
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if ($affected_row > 0) {
            http_response_code(200);
            $reponse = ["message" => "School Data Update successfully", "status" => true];
        } else {
            http_response_code(404);
            $reponse = ["message" => "Data Not Found", "status" => false];
        }
        ...
        http_response_code(500);
        $reponse = ["message" => "School Data Not Update", "status" => false];
    }
} else {
